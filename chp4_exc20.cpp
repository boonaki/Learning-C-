...
int main()
{
    ...
    cout << "enter a name that you inputted in the prompt before, and the computer will give you its age. Type exit when finished.\n";

    while (cin >> name_directory)
    {
        if (name_directory == "exit")
        {
            return 0;
        }
        for (int x = 0; x < names.size(); ++x)
        {
            if (name_directory == names[x])
            {
                cout << ages[x];
            }
        }
